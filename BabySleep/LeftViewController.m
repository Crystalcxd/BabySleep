...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor = HexRGB(0xFFB9B6);
    
    CGFloat width = [[SliderViewController sharedSliderController] LeftSContentOffset];
    
    CGFloat leftPadding = SCREENWIDTH - width;
    
    UIView *titleBG = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREENWIDTH, 100)];
    ...
    NSArray *selectImageArr = [NSArray arrayWithObjects:@"whitenoise_touch" ,@"share_touch",@"volume_touch",@"suggest_touch",nil];
    NSArray *titleArr = [NSArray arrayWithObjects:@"妈咪说故事",@"分享给好友",@"初始音量",@"您的建议", nil];
...
//        [self.view addSubview:label];
        ...
        [clickBtn setTitleColor:HexRGB(0xFF756F) forState:UIControlStateNormal];
        [clickBtn setTitleColor:HexRGB(0xffffff) forState:UIControlStateHighlighted];
    ...
    [btn setTitleColor:HexRGB(0x5C5C5C) forState:UIControlStateNormal];
